document.addEventListener("DOMContentLoaded", () => {
  ...
  const initialCourseTextValues = Array.from(
    document.querySelectorAll(".courses__text")
  ...
  document.addEventListener("click", (e) => {
    if (
      ...
      e.target.classList.contains("buy__btn")
    ) {
      e.preventDefault();
      e.target.classList.contains("buy__btn")
        ? anchorClick(e.target.parentElement)
    ...
    if (e.target.closest(".swiper-slide__btn")) {
      const swiperStartAndFinish = e.target
        .closest(".swiper-slide")
        .querySelector(".swiper-slide__popup");
      let popupContent = document.querySelector(".popup__content");

      popupContent.innerHTML = `
            <a class="popup__close" href="">
                <svg class="popup__close-icon" width="30" height="30" fill="#ff0262" xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512">
  ...
  if (acc) {
    for (let i = 0; i < acc.length; i++) {
      acc[i].addEventListener("click", function () {
        acc[i]
          .querySelector(".moduls__accordeon-btn")
